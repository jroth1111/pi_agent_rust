...
use pi::surface::rpc_types::{RpcOptions, RpcScopedModel};
...
#[test]
fn rpc_orchestration_dispatch_run_persists_worker_failure_report() {
    let harness = TestHarness::new("rpc_orchestration_dispatch_run_persists_worker_failure_report");
    let runtime = asupersync::runtime::RuntimeBuilder::current_thread()
        .build()
        .expect("build test runtime");
    let handle = runtime.handle();
    let run_id = format!("run-e2e-worker-fail-{}", uuid::Uuid::new_v4().simple());
    let (repo_guard, repo_path, head) = setup_inline_repo("pi-e2e-worker-fail");
    ...
    runtime.block_on(async move {
        ...
        let start_cmd = json!({
            ...
            "type": "orchestration.start_run",
            "runId": run_id,
            "objective": "Exercise orchestration worker failure rollback",
            "tasks": [
                {
                    "taskId": "task-e2e-worker-fail",
                    "objective": "Attempt a worker execution that fails",
                    "verifyCommand": "true",
                    "inputSnapshot": head,
                    "acceptanceIds": ["ac-worker-fail"],
                    "plannedTouches": ["Cargo.toml"]
                }
            ],
            "runVerifyCommand": "true"
        ...
        assert!(
            dispatch["error"]
                .as_str()
                .is_some_and(|error| error.contains("scripted provider exhausted its steps"))
        );
...
fn rpc_orchestration_dispatch_run_executes_hierarchical_run() {
    ...
    let runtime = asupersync::runtime::RuntimeBuilder::current_thread()
        ...
        .expect("build test runtime");
    let handle = runtime.handle();
    let run_id = format!("run-e2e-hier-{}", uuid::Uuid::new_v4().simple());
    let (repo_guard, repo_path, head) = setup_inline_repo("pi-e2e-hier");
    ...
    runtime.block_on(async move {
        ...
        let tasks = (0..5)
            .map(|index| {
                let task_id = format!("task-e2e-hier-{index}");
                let mut task = json!({
                    "taskId": task_id,
                    "objective": format!("Complete hierarchical task {index}"),
                    "verifyCommand": "true",
                    "inputSnapshot": head,
                    "acceptanceIds": [format!("ac-hier-{index}")],
                    "plannedTouches": [format!("task-hier-{index}.txt")]
                });
                if index > 0 {
                    task["prerequisites"] = json!([
                        {
                            "taskId": format!("task-e2e-hier-{}", index - 1)
                        }
                    ]);
                }
                task
        ...
        let start_cmd = json!({
            ...
            "type": "orchestration.start_run",
            "runId": run_id,
            "objective": "Exercise hierarchical orchestration dispatch",
            "tasks": tasks,
            "runVerifyCommand": "test -f Cargo.toml",
            "runVerifyTimeoutSec": 30
        })
        .to_string();
        let start = send_recv(&in_tx, &out_rx, &start_cmd, "orchestration.start_run").await;
