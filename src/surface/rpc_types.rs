...
impl RpcReliabilityState {
    ...
    #[cfg(test)]
    pub(crate) fn set_external_lease_provider(
        &mut self,
        provider: Arc<dyn reliability::LeaseProvider>,
    ) {
        self.leases.set_external_provider(provider);
    }

    #[cfg(test)]
    pub(crate) const fn mode_blocks(&self) -> bool {
    ...
    #[cfg(test)]
    pub(crate) fn get_or_create_task(
        &mut self,
    ...
    #[cfg(test)]
    pub(crate) fn task_counts_for(&self, task_ids: &[String]) -> BTreeMap<String, usize> {
    ...
    #[cfg(test)]
    pub(crate) fn waiting_on_for_task(&self, task_id: &str) -> Vec<String> {
    ...
    #[cfg(test)]
    pub(crate) fn evaluate_dag_unblock(&mut self) -> reliability::DagEvaluation {
        ReliabilityService::evaluate_dag_unblock(self)
    }

    #[cfg(test)]
    pub(crate) fn promote_recoverable_due(&mut self) -> Vec<reliability::RecoveryAction> {
    ...
    #[cfg(test)]
    pub(crate) fn append_evidence(&mut self, req: AppendEvidenceRequest) -> Result<EvidenceRecord> {
        ReliabilityService::append_evidence_state(self, req)
    }

    #[cfg(test)]
    pub(crate) fn submit_task(&mut self, req: SubmitTaskRequest) -> Result<SubmitTaskResponse> {
        ReliabilityService::submit_task_state(self, req)
    }

    #[cfg(test)]
    pub(crate) fn resolve_blocker(&mut self, report: BlockerReport) -> Result<String> {
        ReliabilityService::resolve_blocker(self, report)
    }

    #[cfg(test)]
    pub(crate) fn query_artifact(&self, query: ArtifactQuery) -> Result<Vec<String>> {
        ReliabilityService::query_artifact(self, query)
    }

    #[cfg(test)]
    pub(crate) fn load_artifact_text(&self, artifact_id: &str) -> Result<String> {
    ...
    #[cfg(test)]
    pub(crate) fn get_state_digest(&mut self, task_id: &str) -> Result<StateDigest> {
